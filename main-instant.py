...
path=r"D:/Human Image Proj/dataset_final/dataset/captured_img.jpg"
...
# Convert into grayscale
    gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    face_cascade = cv.CascadeClassifier('D:/Human Image Proj/dataset_final/dataset/haarcascade_frontalface_default.xml')
...
#This module detects the skin portion in the input image.
    ...
    image = cv.imread(r"D:/Human Image Proj/dataset_final/dataset/face.jpg")
    ...
    cv.imwrite(r"D:/Human Image Proj/dataset_final/dataset/detectedImage.png", 
...
#This module collects the color and checks with the provided range and classifies accordingly.
    ...
    image = cv.imread(r'D:/Human Image Proj/dataset_final/dataset/detectedImage.png')
    ...
    if 16777215>fc>12619362: #fair range
        ...
        cv.putText(img,'Indian, Skin Tone: MILD',(10,50), font, 1,(0,255,0),2)
        cv.imshow("Result",img)
        cv.waitKey(0)
    
    else:
        print("Dark")    
        cv.putText(img,'Indian, Skin Tone: DARK',(10,50), font, 1,(0,255,0),2)
        cv.imshow("Result",img)
        cv.waitKey(0)


elif(huver=='Non Human'):
    original=cv.imread(path)
    font = cv.FONT_HERSHEY_TRIPLEX
    cv.putText(original,'Skin tone not detected as it is  Non-Human',(10,50), font, 1,(0,255,0),2)
    ...
    font = cv.FONT_HERSHEY_TRIPLEX
    cv.putText(original,'Skin tone not detected as person is Non-Indian',(10,50), font, 1,(0,255,0),2)
    cv.imshow("Result",original)
    cv.waitKey(0)
